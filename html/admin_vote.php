...
<body>
    ...
    $query = "SELECT s.name AS show_name, c.id AS contestant_id, c.name AS contestant_name, c.image_url, c.description, c.status, 
              ...
              LEFT JOIN (SELECT contestant_id, SUM(vote_count) AS vote_count FROM votes GROUP BY contestant_id) v ON c.id = v.contestant_id
              ORDER BY s.name, v.vote_count DESC";
