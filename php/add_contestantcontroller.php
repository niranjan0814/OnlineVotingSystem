...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($show_id) || empty($name)) {
        echo json_encode(['success' => false, 'message' => 'Show ID and name are required']);
        exit;
    }

    $insert_query = "INSERT INTO contestants (show_id, name, image_url, description, status) VALUES (?, ?, ?, ?, ?)";
    $insert_stmt = mysqli_prepare($con, $insert_query);
    mysqli_stmt_bind_param($insert_stmt, "issss", $show_id, $name, $image_url, $description, $status);

    if (mysqli_stmt_execute($insert_stmt)) {
        echo json_encode(['success' => true, 'message' => 'Contestant added successfully']);
    } else {
        echo json_encode(['success' => false, 'message' => 'Error adding contestant: ' . mysqli_error($con)]);
    }
